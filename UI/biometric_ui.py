...
class BiometricAuthUI:
    ...
    def create_authentication_tab(self):
        ...
        self.auth_face_threshold.set(0.7)
        ...
        self.auth_voice_threshold.set(0.60)
        ...
        try:
            ...
            if face_response.status_code == 200:
                ...
                self.safe_messagebox(
                    "error", "Error", f"Face registration failed: {error}"
                )
            ...
            if voice_response.status_code == 200:
                ...
                self.safe_messagebox(
                    "error", "Error", f"Voice registration failed: {error}"
                )
            ...
            self.safe_messagebox(
                "info", "Success", f"User '{username}' registered successfully!"
            )
            ...
            self.safe_messagebox(
                "error", "Error", f"Cannot connect to services: {str(e)}"
            )
            ...
            self.safe_messagebox(
                "error", "Error", f"Registration failed: {str(e)}"
            )
    ...
    def authenticate_user(self):
        ...
        self.log_message(
            f"Initiating authentication with DS fusion "
            f"(Face Threshold: {face_threshold:.2f}, Voice Threshold: {voice_threshold:.2f})...",
    ...
    def _authenticate_user_thread(self, face_threshold, voice_threshold):
        """Background thread for authentication with Dempster–Shafer fusion"""
        try:
            self.log_message(
                "Initiating authentication with Dempster–Shafer (DS) fusion...",
                is_registration=False,
            )

            # Prepare data for backend and DS fusion
            frames_b64 = self.frames_to_base64(
                self.recorded_frames, sample_count=8
            )
            audio_b64 = self.audio_to_base64()

            # --- Compute modality reliabilities from captured data ---

            # Face reliability: use number of raw frames (not only sampled),
            # and basic brightness/contrast analysis.
            num_frames = len(self.recorded_frames)
            face_reliability = (
                self.auth_handler.authenticator.compute_face_reliability(
                    self.recorded_frames, num_frames
                )
            )

            # Voice reliability: use duration (seconds) and RMS energy.
            audio_duration = len(self.audio_frames) * 1024 / 16000.0
            voice_reliability = (
                self.auth_handler.authenticator.compute_voice_reliability(
                    self.audio_frames, audio_duration
                )
            )

            self.log_message(
                f"Face reliability: {face_reliability:.3f}, ",
                is_registration=False,
            )
            self.log_message(
                f"Voice reliability: {voice_reliability:.3f}",
                is_registration=False,
            )

            # Use DS fusion authentication
            ...
            self.log_message(
                ...
                f"Reliability={face_ev.get('reliability', 0):.3f}",
                f"as {face_ev.get('username')}",
                is_registration=False,
            )
            self.log_message(
                f"🎤 Voice: Score={voice_ev.get('score', 0):.3f}, "
                f"Identified={voice_ev.get('identified')}, "
                f"Reliability={voice_ev.get('reliability', 0):.3f}",
                f"as {voice_ev.get('username')}",
                is_registration=False,
            )

            # Log DS fusion metrics
            ds = fusion_result.get("ds", {})
            self.log_message("=" * 50, is_registration=False)
            self.log_message(
                f"DS Fusion → Bel(Genuine)={ds.get('belief_genuine', 0):.3f}, "
                f"Pl(Genuine)={ds.get('plausibility_genuine', 0):.3f}, "
                f"Conflict={ds.get('conflict', 0):.3f}",
                is_registration=False,
            )

            # Show detailed report (DS explanation)
            ...
            # Final decision from fusion result
            if fusion_result.get("decision", False):
                self.auth_status_label.config(
                    text="Authenticated (DS Fusion)", foreground="green"
                )
                self.safe_messagebox(
                    "info",
                    "Authenticated",
                ...
                self.safe_messagebox(
                    ...
                    "Dempster-Shafer fusion did not support a genuine match.",
                )
            ...
            self.safe_messagebox(
                "error", "Error", f"Authentication failed: {str(e)}"
            )
    ...
    def log_message(self, *args, **kwargs):
        ...
        Accepts:
        log_message(message)
        log_message(message, is_registration=True/False)
        ...
        # 2) Extract is_registration
        if "is_registration" in kwargs:
            is_registration = kwargs["is_registration"]
        elif len(args) >= 2:
            # In case someone called log_message(msg, True/False)
            is_registration = bool(args[1])
        else:
            is_registration = False
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Biometric UI / Testing")
    parser.add_argument(
        "--testing",
        action="store_true",
        help="Run bulk CLI testing instead of starting the Tkinter UI",
    )
    parser.add_argument(
        "--data-root",
        default="biometric_data",
        help="Root folder containing faces/ and voices/ subfolders",
    )
    parser.add_argument(
        "--max-test-samples",
        type=int,
        default=5,
        help="Maximum number of test samples per user",
    )
    parser.add_argument(
        "--min-train-samples",
        type=int,
        default=3,
        help="Minimum number of samples per user required for training",
    )
    args = parser.parse_args()

    if args.testing:
        # CLI bulk testing mode
        from ui_fusion_integration import UIAuthenticationHandler, BulkTester

        # Use the same URLs you configure for the UI
        face_service_url = "http://localhost:5001"
        voice_service_url = "http://localhost:5002"

        handler = UIAuthenticationHandler(
            face_service_url=face_service_url,
            voice_service_url=voice_service_url,
        )
        tester = BulkTester(handler)
        tester.run_bulk_testing(
